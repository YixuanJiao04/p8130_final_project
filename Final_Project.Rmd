...
![](plots/bodyfat_brozek_pairs.png){width=70% height=70%}
...
ggplot(data = vif_df,aes(y = variable, x = vif_values)) +
  ...
  labs(x = "VIF Value",
       ...
       caption = "Fig3: VIF Value of Predictors")
```
...
ggplot(data = vif_df,aes(y = variable, x = vif_values)) +
  ...
  labs(x = "VIF Value",
       ...
       caption = "VIF Value After Removing Highly Correlated Predictors")
```
...
cp <- leaps(x = mat[,c(-1)] , y = mat[,1], nbest = 1, method = "Cp")$Cp
adjr2 <- leaps(x = mat[,c(-1)] , y = mat[,1], nbest = 1, method = "adjr2")$adjr2
```
...
final_mod2 <- update(stepwise.fit, . ~ . + bicep + neck*abdomen)
par(mfrow = c(2,2))
```
